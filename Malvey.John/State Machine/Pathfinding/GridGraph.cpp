...
void GridGraph::init()
{
	if( mpGrid != NULL )
	{
		mNumGridIndices = mpGrid->getGridWidth() * mpGrid->getGridHeight();

		//size the node vector
		mNodes.resize( mNumGridIndices, NULL );
		
		//go through all elements of the grid and make Nodes for them - also add them to Node vector
		for( int i=0; i<mNumGridIndices; i++ )
		{
			//create a Node for this location
			Node* pNode = new Node( i );
			pNode->setPos(mpGrid->getULCornerOfSquare(i));
			//add to NodeVector
			mNodes[i] = pNode;
		}
			...
			for( unsigned int adjIndex = 0; adjIndex < adjacencies.size(); adjIndex++ )
			{
				...
				if( mpGrid->getValueAtIndex(adjacencies[adjIndex]) != BLOCKING_VALUE )
				{
					Node* pToNode = mNodes[ adjacencies[adjIndex] ];//find to node
					 ...
					 Connection* pConnection = new Connection( pFromNode, pToNode, adjIndex % 2 == 0 /*diagonal fix*/ ? 1.0f : 2.0f);//create a connection
