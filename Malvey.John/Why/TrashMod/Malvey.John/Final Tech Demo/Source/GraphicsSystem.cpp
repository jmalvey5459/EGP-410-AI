...
void GraphicsSystem::draw(GraphicsBuffer& dest, const Sprite& aSprite, float dx, float dy, float rotationInRadians /*= 0*/, int flip /* =0*/, int flags /*= 0*/ )
{
	...
	case(0):
		...
		break;
	case(1):
		SDL_RenderCopyEx(pRenderer, aSprite.getBuffer()->mpTexture, &srcRect, &destRect, rotationInDegrees, NULL, SDL_FLIP_HORIZONTAL);
