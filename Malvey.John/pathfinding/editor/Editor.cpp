...
const std::string FILE_NAME = "../game/pathgrid.txt";
...
void Editor::processLoop()
{
	...
	if (SDL_GetMouseState(&x, &y) & SDL_BUTTON(SDL_BUTTON_LEFT))
	{
		if (!lastClickLeft)
		{
			switch (mpGrid->getValueAtPixelXY(x, y))
			{
			case(CLEAR_VALUE):
			{
			...
			case(BLOCKING_VALUE):
			{
				mpGrid->setValueAtPixelXY(x, y, TERRAIN_SAND);
				mpGridVisualizer->setModified();
				break;
			}
			case(TERRAIN_SAND):
			{
				mpGrid->setValueAtPixelXY(x, y, TERRAIN_WATER);
				mpGridVisualizer->setModified();
				break;
			}
			case(TERRAIN_WATER):
